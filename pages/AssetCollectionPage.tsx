...
import { User, CollectionAsset } from '../types';
...
interface AssetCollectionPageProps {
  ...
  error: string | null;
  onRefresh: () => void;
  onDelete: (asset: CollectionAsset) => void;
...
const AssetCollectionPage: React.FC<AssetCollectionPageProps> = ({ user, assets, isLoading, error, onRefresh, onDelete, deletingAssetId }) => {
  const [downloadingId, setDownloadingId] = useState<string | null>(null);
  
  const handleDownload = async (asset: CollectionAsset) => {
    ...
    try {
        ...
        link.href = url;
        link.download = `zola-ai-collection-${asset.id.substring(0, 8)}.png`;
                ...
                <div key={asset.id} className="group relative aspect-[4/5] bg-gray-100 rounded-xl shadow-lg overflow-hidden border border-gray-200">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <div className="absolute bottom-0 left-0 p-4">
                        <p className="text-white font-bold text-sm capitalize">{asset.item_name}</p>
                        <p className="text-white/80 text-xs">{asset.item_category} ({asset.asset_type})</p>
                      </div>
                  </div>
                </div>
              );
